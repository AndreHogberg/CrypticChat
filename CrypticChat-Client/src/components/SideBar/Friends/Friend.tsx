...
export default function Friend({ name, friendId, chatRoom }: Props) {
  return (
    <Link
      className="flex flex-row w-full"
      to={`/chat/${friendId}?chatRoom=${chatRoom}`}
    >
      <div className="flex flex-row w-full h-10 space-x-3 p-1 justify-start items-center hover:bg-purple-700 rounded-md hover: cursor-pointer">
        <img
          ...
          src={`https://avatars.dicebear.com/api/adventurer-neutral/${friendId}.svg`}
        />
        <div>{name}</div>
      </div>
    </Link>
